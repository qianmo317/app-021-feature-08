...
8. **导出与打印**：按周座位表 CSV、公平性统计 CSV（带 BOM，Excel 直接打开）；打印页支持按周勾选与起止周筛选（默认全选），先显示总页数与每页对应周次，再按 A4 纵向渲染；座位图超出一页时按排自动续页，每页都有讲台方向条、周次页码（如第 3 周 2/2）与标记说明页脚。

## 5. 进阶功能
- 座位特殊标记（`stage_side` 讲台侧）的界面化标注。
- 多套种子对比择优（目前只能「换种子」逐个看）。
- 学期中转入 / 转出学生的局部重排（现在改名单会清空已生成结果，需整体重新生成）。
- 打印页的自动缩放（目前超页按排续页，未做整图缩放到一页）。
...
/class/:id/print           打印座位表（周次勾选 + 起止周筛选，A4 纵向，超页自动续页）
```
...
- 打印态座位图放大字号（姓名 15pt），每张物理页带标题、讲台方向条与标记说明页脚；`@page { size: A4 portrait; margin: 8mm }`，每页 `page-break-after: always`（`styles.css` 打印节）。
- 打印筛选：周次复选 chip（默认全选）+ 起始/结束周下拉（反向选择时自动拉齐另一端），页面实时显示总页数与「第 N 周 · x/y」逐页清单；空选时禁用打印按钮（`src/pages/Print.tsx`）。
- 超页续页：`Print.tsx` 用与打印页等宽（194mm）的隐藏探针页实测每一排座位的 px 高度，按 281mm 可打印区减去页眉/讲台条/页脚贪心切排，结果缓存（布局或人数变化时失效）；探针逐周串行挂载，避免 20 周全选时同时渲染。续页由 `SeatGrid` 的 `rowRange` 只渲染对应排，并重复讲台条、续页页眉（续上页（第 a–b 排）、第 N 周 x/y）与标记说明页脚。
...
- 打印续页按隐藏页实测的固定排高切分（留 4mm 安全余量），字体回退异常或浏览器打印缩放与屏幕 DPI 偏差极大时仍可能略有出入；未做整图自动缩放到一页。
